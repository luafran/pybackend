...
class CircuitBreaker(object):
    ...
    def success(self):
        """Update the circuit breaker with a success event."""
...
class CircuitBreakerSet(object):
    ...
    def context(self, circuit_id):
        """Return a circuit breaker for the given ID."""
        if circuit_id not in self.circuits:
            self.circuits[circuit_id] = self.factory(self.clock, self.log.getChild(circuit_id),
